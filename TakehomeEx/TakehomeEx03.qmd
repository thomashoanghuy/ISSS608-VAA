...
Finally we created another columns in our dataset, and we can add the list of numeric years back into our data set as new columns.
...
group_by(month) %>%
summarise(amk_monthly_avg_per_month = mean(price_per_sqm))

amk_monthly_avg
```

```{r}
p = ggplot(amk_monthly_avg, aes(x= amk_monthly_avg$month, y=amk_monthly_avg$amk_monthly_avg_per_month , group = 1)) +
...
## LEASE REMAINS NUMBER OF YEARS VERSUS AVERGE PRICE PER SQM

```{r}
ggplot(data= overall_data, mapping = aes(x = overall_data$lease_remains, y = overall_data$price_per_sqm)) + 
  geom_smooth() +
  theme_bw()+
  xlab("Years of Lease Remains") + ylab("Average Price per Square meters")
```
